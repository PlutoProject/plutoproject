...
@Suppress("UNUSED")
internal object VelocityFlagCommand {
    private val registry get() = koinGet<FlagRegistry>()

    // ---- set ----

    @Command("flagv set global <key> <value>")
    @Permission(PERMISSION_COMMAND_FLAG)
    suspend fun CommandSource.setGlobal(
        @Argument("key", parserName = "flag-key") key: FlagKey<*>,
        @Argument("value") @Greedy value: String,
    ) = set(Scope.GLOBAL, key, value)

    @Command("flagv set server <key> <value>")
    @Permission(PERMISSION_COMMAND_FLAG)
    suspend fun CommandSource.setServer(
        @Argument("key", parserName = "flag-key") key: FlagKey<*>,
        @Argument("value") @Greedy value: String,
    ) = set(Scope.SERVER, key, value)

    @Command("flagv set player <player> <key> <value>")
    @Permission(PERMISSION_COMMAND_FLAG)
    suspend fun CommandSource.setPlayer(
    ...
    @Command("flagv clear global <key>")
    @Permission(PERMISSION_COMMAND_FLAG)
    suspend fun CommandSource.clearGlobal(
        @Argument("key", parserName = "flag-key") key: FlagKey<*>,
    ) = clear(Scope.GLOBAL, key)

    @Command("flagv clear server <key>")
    @Permission(PERMISSION_COMMAND_FLAG)
    suspend fun CommandSource.clearServer(
        @Argument("key", parserName = "flag-key") key: FlagKey<*>,
    ) = clear(Scope.SERVER, key)

    @Command("flagv clear player <player> <key>")
    @Permission(PERMISSION_COMMAND_FLAG)
    suspend fun CommandSource.clearPlayer(
        @Argument("player", parserName = "flag-player") player: UUID,
        @Argument("key", parserName = "flag-key") key: FlagKey<*>,
    ...
    @Command("flagv get <key> [flags]")
    @Permission(PERMISSION_COMMAND_FLAG)
    suspend fun CommandSource.get(
        @Argument("key", parserName = "flag-key") key: FlagKey<*>,
        @Argument("flags") @Greedy flags: String?,
    ) {
        val filter = when (val selection = parseScopeSelection(flags, allowWorld = false)) {
    ...
    @Command("flagv list [flags]")
    @Permission(PERMISSION_COMMAND_FLAG)
    suspend fun CommandSource.list(@Argument("flags") @Greedy flags: String?) {
        val filter = when (val selection = parseScopeSelection(flags, allowWorld = false)) {
    ...
    private suspend fun CommandSource.set(scope: Scope, key: FlagKey<*>, value: String) {
    ...
    private suspend fun CommandSource.clear(scope: Scope, key: FlagKey<*>) {
