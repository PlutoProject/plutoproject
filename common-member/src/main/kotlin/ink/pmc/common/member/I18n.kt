...
val MEMBER_LOOKUP_FAILED_UNKNOWN_AUTH_TYPE
...
val MEMBER_EXEMPT_WHITELIST_FAILED_NOT_WHITELISTED
    get() = Component.text("玩家 <player> 未获得白名单").color(mochaMaroon)

val MEMBER_GRAND_WHITELIST_SUCCEED
    get() = Component.text("已成功为玩家 <player> 发放白名单").color(mochaGreen)

val MEMBER_GRANT_WHITELIST_FAILED_ALREADY_WHITELISTED
    get() = Component.text("玩家 <player> 已获得白名单").color(mochaMaroon)

val MEMBER_LINK_BE_FAILED_BE_ONLY
    get() = Component.text("该是仅基岩版进入的玩家，无法进行绑定").color(mochaMaroon)

val MEMBER_LINK_BE_FAILED_ACCOUNT_ALREADY_EXISTED
    get() = Component.text("绑定失败，账号 <gamertag> (XUID: <xuid>) 已经被玩家 <other> 绑定了").color(mochaMaroon)

val MEMBER_LINK_BE_FAILED_ALREADY_LINKED
    get() = Component.text("绑定失败，该玩家已经绑定过基岩版账号").color(mochaMaroon)

val MEMBER_LINK_BE_FAILED_NOT_EXISTED
    get() = Component.text("绑定失败，该基岩版账号不存在")

val MEMBER_LINK_BE_SUCCEED
    get() = Component.text("已成功为玩家 <player> 绑定基岩版账号").color(mochaGreen)

val MEMBER_UNLINK_BE_FAILED_ALREADY_BE_ONLY
    get() = Component.text("该是仅基岩版进入的玩家，无法进行解绑").color(mochaMaroon)
        .appendNewline()
        .append(
            Component.text("如果你执意要这么做，请加上 ").color(mochaSubtext0).append(
                Component.text("--force ").color(
                    mochaFlamingo
                ).append(Component.text("标签").color(mochaSubtext0))
            )
        )

val MEMBER_UNLINK_BE_FAILED_NOT_LINKED
    get() = Component.text("解绑失败，该玩家未绑定基岩版账号").color(mochaMaroon)

val MEMBER_UNLINK_BE_SUCCEED
...
val MEMBER_LOOKUP
    get() = Component.text("UID：").color(mochaText).append(Component.text("<uid>").color(mochaYellow))
        ...
        .appendNewline()
        .append(
            Component.text("白名单状态：").color(mochaText)
        ...
        .appendNewline()
        .append(
            Component.text("上次进入时间：").color(mochaText).append(Component.text("<lastJoinedAt>").color(mochaYellow))
        )
        .appendNewline()
        .append(
            Component.text("上次退出时间：").color(mochaText).append(Component.text("<lastQuitedAt>").color(mochaYellow))
        )
        .appendNewline()
        .append(
            Component.text("数据容器：").color(mochaText).append(Component.text("<dataContainer>").color(mochaYellow))
        )
        .appendNewline()
        .append(
            Component.text("基岩版账号：").color(mochaText).append(Component.text("<bedrockAccount>").color(mochaYellow))
        )
        .appendNewline()
